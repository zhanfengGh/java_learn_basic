...
public class MethodInvoke {
	...
	public static void printAllFieldsAndMethodsAndConstructor(Class<?> clazz){
		while (clazz!=null){
			System.out.println();
	...
	private static Method tryToGetMethod(Object obj, String methodName, boolean argsOfMethodUsePrimitive, Object...args){
		...
		for (int i=0;i<args.length;i++){
			...
			if (argsOfMethodUsePrimitive){
				argsType[i]=changeToPrimitiveTypeIfHasAny(temp);
			} else{
				argsType[i]=temp;
			}
		}
	 ...
	 * @return
	 */
	private static Class<?> changeToPrimitiveTypeIfHasAny(Class<?> clazz) {
	...
	public static List<Class<?>> getAllSuperObject(Object obj, boolean primitive){
		...
		Class<?> clazz=obj.getClass();
		if (primitive){
			clazz=changeToPrimitiveTypeIfHasAny(clazz);
		}
		while(clazz!=null){
			all.add(clazz);
			clazz=clazz.getSuperclass();
